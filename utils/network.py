...
class D3(nn.Module):
  ...
  def forward(self, x, sp_inputs):
    ...
    # densenet1
    h =  nn.functional.interpolate(sp_inputs, scale_factor=0.5,mode="bilinear",align_corners=True,recompute_scale_factor=True)
    x1 = torch.relu(self.dense1_conv1(x))
    x1 = self.densenet1(x1, h)
    x1 = torch.relu(self.dense1_conv2(x1))
    
    # skip densenet2
    h2 = nn.functional.interpolate(h, scale_factor=0.5,mode="bilinear",align_corners=True,recompute_scale_factor=True)
    ...
    # skip densenet3
    h3 = nn.functional.interpolate(h2, scale_factor=0.5,mode="bilinear",align_corners=True,recompute_scale_factor=True)
    ...
    # skip densenet4
    h4 = nn.functional.interpolate(h3, scale_factor=0.5,mode="bilinear",align_corners=True,recompute_scale_factor=True)
    ...
    # densenet5
    h5 = nn.functional.interpolate(h4, scale_factor=0.5,mode="bilinear",align_corners=True,recompute_scale_factor=True)
